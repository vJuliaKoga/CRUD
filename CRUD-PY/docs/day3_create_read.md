...
curl -X POST http://localhost:8000/books/ \
  ...
  -d '{"title": "リーダブルコード", "author": "Dustin Boswell", "publisher": "オライリージャパン"}'
```

Windows PowerShellの場合:

```powershell
Invoke-WebRequest -Uri "http://localhost:8000/books/" -Method POST -ContentType "application/json" -Body '{"title": "リーダブルコード", "author": "Dustin Boswell", "publisher": "オライリージャパン"}'
```
...
curl -X POST http://localhost:8000/books/ \
  -H "Content-Type: application/json" \
  -d '{"title": "プログラミング作法", "author": "Brian Kernighan", "publisher": "ASCII", "isbn": "9784756136497"}'
```
